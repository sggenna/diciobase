...
export default function SwipeToast({
  ...
  const close = (why: string) => {
    ...
    if (drag.current) { pendingClose.current = why; return; }
    anim.current?.pause();
    reason.current = why;
  ...
  useEffect(() => {
    ...
    anim.current?.cancel();
    const a = fuseRef.current.animate(BURN as Keyframe[], { duration, easing: 'linear', fill: 'forwards' });
    a.onfinish = () => close('timeout');
    anim.current = a;
    syncFuse();
    return () => { a.onfinish = null; a.pause(); };
  }, [phase, duration]); // eslint-disable-line

  useEffect(() => {
    if (!pauseOnHover) { flags.current.hover = false; syncFuse(); }
  }, [pauseOnHover]); // eslint-disable-line

  useEffect(() => {
    const onVisibility = () => { flags.current.hidden = document.hidden; syncFuse(); };
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    y.stop();
    drag.current = { id: e.pointerId, startY: e.clientY, grab: null, moved: false, hist: [[performance.now(), y.get()]] };
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (d.grab === null) {
      ...
      if (cardRef.current) (cardRef.current as HTMLElement & { dataset: DOMStringMap }).dataset.swiping = '';
    }
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    if (!d || d.id !== e.pointerId) return;
    drag.current = null;
    if (cardRef.current) delete (cardRef.current as HTMLElement & { dataset: DOMStringMap }).dataset.swiping;
    try { cardRef.current?.releasePointerCapture(e.pointerId); } catch {}
    flags.current.interacting = false;
    const dy = y.get();
    const v = velocityOf(d.hist);
    if (dy > 0 && (v > FLICK || (dy >= swipeDistance && v >= 0))) { swipeOut(dy, v); return; }
    if (d.moved) {
      if (reduce) {
        animate(y, 0, { duration: 0.2 });
      } else {
        animate(y, 0, { type: 'spring', duration: 0.5, bounce: settleBounce, velocity: v * 1000 });
      }
    }
      ...
      <div className="swipe-toast__gate">
        <div className="swipe-toast__lift">
          <motion.div
            ...
            onPointerEnter={(e) => {
              if (pauseOnHover && e.pointerType === 'mouse') { flags.current.hover = true; syncFuse(); }
            }}
            onPointerLeave={(e) => {
              if (e.pointerType === 'mouse') { flags.current.hover = false; syncFuse(); }
            }}
            onFocus={() => { flags.current.focus = true; syncFuse(); }}
            onBlur={(e) => {
              if (!e.currentTarget.contains(e.relatedTarget as Node)) { flags.current.focus = false; syncFuse(); }
            }}
            onKeyDown={(e) => {
              if (e.key === 'Enter' || e.key === ' ') lastInput.current = 'keyboard';
              if (e.key === 'Escape' && dismissible) { e.stopPropagation(); close('escape'); }
            }}
          >
            {icon && <span className="swipe-toast__icon" aria-hidden="true">{icon}</span>}
            ...
            {actionLabel ? (
              <button type="button" className="swipe-toast__action"
                onClick={() => { latest.current.onAction?.(); close('action'); }}>
                {actionLabel}
              </button>
            ) : null}
            {closeButton ? (
              <button type="button" className="swipe-toast__close" aria-label="Close"
                onClick={() => close('close')}>
